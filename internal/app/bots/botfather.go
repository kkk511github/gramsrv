...
var pickerPrompts = map[string]string{
	...
	botFatherCmdSetLogin:       "Choose a bot to configure SafeLink Login for. Send the bot's username:",
	botFatherCmdLoginInfo:      "Choose a bot whose SafeLink Login configuration you want to inspect:",
...
func (s *Service) handleChooseBot(ctx context.Context, state domain.BotChatState, text string) botReply {
	...
	case botFatherCmdLoginInfo:
		...
		if !found {
			return botReply{Text: fmt.Sprintf("SafeLink Login is not configured for @%s. Use /setlogin to create it.", chosen.Username)}
		}
	...
	case botFatherCmdResetLogin:
		...
		if errors.Is(err, domain.ErrTelegramLoginClientInvalid) {
			return botReply{Text: fmt.Sprintf("SafeLink Login is not configured for @%s. Use /setlogin first.", chosen.Username)}
		}
		if err != nil {
...
func formatTelegramLoginConfiguration(username string, configuration telegramloginapp.ClientConfiguration) string {
	...
	fmt.Fprintf(&out, "SafeLink Login for @%s\nClient ID: %s\nStatus: %s\nSigning algorithm: %s\nSecret version: %d",
...
func (s *Service) applyTelegramLoginConfiguration(ctx context.Context, botID int64, username, text string) (botReply, error) {
	if s.telegramLogin == nil {
		return botReply{Text: "SafeLink Login is not enabled on this server."}, domain.ErrTelegramLoginClientDisabled
	}
	...
	if len(fields) == 1 {
		...
		case "disable":
			...
			return botReply{Text: fmt.Sprintf("SafeLink Login is disabled for @%s. Pending requests can no longer be approved or exchanged.", username)}, nil
		}
	}
