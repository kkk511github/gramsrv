...
func TestStickersBotPublishStickerPack(t *testing.T) {
	...
	if !strings.Contains(reply, "https://safelink.chat/addstickers/fresh_pack") {
		t.Fatalf("publish reply = %q, want addstickers link", reply)
	}
	publishReply := latestStickersReply(t, messages, owner.ID)
	assertReplyEntityText(t, publishReply, domain.MessageEntityURL, "https://safelink.chat/addstickers/fresh_pack")
...
func TestStickersBotPublishUploadedTGS(t *testing.T) {
	...
	if !strings.Contains(reply, "https://safelink.chat/addemoji/local_emoji") {
...
func TestStickersBotPublishUploadedLottieJSON(t *testing.T) {
	...
	if !strings.Contains(reply, "https://safelink.chat/addstickers/lottie_pack") {
...
func TestStickersBotPublishCustomEmojiPack(t *testing.T) {
	...
	if !strings.Contains(reply, "https://safelink.chat/addemoji/emoji_pack") {
...
func TestStickersBotAddStickerToExistingPack(t *testing.T) {
	...
	if reply := sendTextToStickers(t, svc, messages, owner, "https://safelink.chat/addstickers/fresh_pack"); !strings.Contains(reply, "Selected Fresh Pack") {
	...
	if !strings.Contains(reply, "Done. Added to Fresh Pack") || !strings.Contains(reply, "https://safelink.chat/addstickers/fresh_pack") {
...
func TestStickersBotDeleteStickerFromExistingPack(t *testing.T) {
	...
	if !strings.Contains(reply, "Done. Removed from Old Pack") || !strings.Contains(reply, "https://safelink.chat/addstickers/old_pack") {
...
func TestStickersBotDeleteCustomEmojiEntityFromExistingPack(t *testing.T) {
	...
	if !strings.Contains(reply, "Done. Removed from Emoji Pack") || !strings.Contains(reply, "https://safelink.chat/addemoji/emoji_pack") {
