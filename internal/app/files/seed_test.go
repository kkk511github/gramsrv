...
func TestSeedMediaUsesInputSetTypeForSystemSets(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			storageDocID := seedDocumentStorageID(tc.docID)
			if set.ID != tc.id || set.ShortName != tc.shortName || len(set.DocumentIDs) != 1 || set.DocumentIDs[0] != storageDocID {
				t.Fatalf("set = %+v, want id=%d short=%s doc=%d", set, tc.id, tc.shortName, storageDocID)
			}
			...
			if resolved.ID != tc.id || len(docs) != 1 || docs[0].ID != storageDocID {
				t.Fatalf("resolved = set %d docs %+v, want set %d doc %d", resolved.ID, docs, tc.id, storageDocID)
			}
		})
	}
}
...
func TestSeedMediaReclassifiesExistingSystemSetWhenHashUnchanged(t *testing.T) {
	...
	storageDocID := seedDocumentStorageID(docID)
	if len(set.DocumentIDs) != 1 || set.DocumentIDs[0] != storageDocID {
		t.Fatalf("set document ids = %+v, want [%d]", set.DocumentIDs, storageDocID)
	}
}
