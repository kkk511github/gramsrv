...
func TestGetFileResolvesStickerSetThumbToThumbDocument(t *testing.T) {
	...
	media := newFakeMediaStore()
	if err := media.PutFileBlob(ctx, domain.FileBlob{LocationKey: "doc:99", Backend: domain.MediaBackendLocalFS, ObjectKey: objectKey, Size: 8, MimeType: "application/x-tgsticker"}); err != nil {
		t.Fatalf("put blob: %v", err)
	}
