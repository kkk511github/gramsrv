...
func TestAuthorizedDeviceTakesPriorityOverConfiguredLoginEmail(t *testing.T) {
	ctx := context.Background()
	users := memory.NewUserStore()
	u, err := users.Create(ctx, domain.User{Phone: "15550009217"})
	if err != nil {
		t.Fatalf("create user: %v", err)
	}
	authz := memory.NewAuthorizationStore()
	if err := authz.Bind(ctx, domain.Authorization{AuthKeyID: [8]byte{0x92, 0x17}, UserID: u.ID}); err != nil {
		t.Fatalf("bind existing device: %v", err)
	}
	emails := &testLoginEmailStore{emails: map[string]string{"15550009217": "secure@example.test"}}
	mailSender := &testMailSender{}
	delivery := &captureLoginCodeDelivery{}
	svc := NewService(users, authz, memory.NewCodeStore(), nil, nil, "12345",
		WithLoginEmail(LoginEmailOptions{Enabled: true, CodeLength: 5, Store: emails, Sender: mailSender}),
		WithLoginCodeDelivery(delivery),
	)

	hash, err := svc.SendCode(ctx, "15550009217")
	if err != nil {
		t.Fatalf("SendCode: %v", err)
	}
	if hash == "" || len(delivery.requests) != 1 || delivery.requests[0].UserID != u.ID {
		t.Fatalf("app delivery hash=%q requests=%+v", hash, delivery.requests)
	}
	if mailSender.to != "" || mailSender.code != "" {
		t.Fatalf("email unexpectedly sent to/code=%q/%q", mailSender.to, mailSender.code)
	}
}
