...
func NewService(users store.UserStore, auths store.AuthorizationStore, codes store.CodeStore, authKeys store.AuthKeyStore, tempKeys store.TempAuthKeyBindingStore, fixedCode string, opts ...Option) *Service {
	s := &Service{users: users, auths: auths, codes: codes, authKeys: authKeys, tempKeys: tempKeys, fixedCode: fixedCode, codeTTL: 5 * time.Minute, codeMaxAttempts: 5, loginEmailCodeLength: defaultLoginEmailCodeLength, phoneCodeLength: 5}
...
func (s *Service) SendCode(ctx context.Context, phone string) (string, error) {
	...
	if found {
		...
		hasDevice, err := s.hasFullyAuthorizedDevice(ctx, existing.ID)
		if err != nil {
			return "", fmt.Errorf("lookup authorized login-code device: %w", err)
		}
		if hasDevice {
...
func (s *Service) hasFullyAuthorizedDevice(ctx context.Context, userID int64) (bool, error) {
	if s == nil || s.auths == nil || userID == 0 {
		return false, nil
	}
	authorizations, err := s.auths.ListByUser(ctx, userID)
	if err != nil {
		return false, err
	}
