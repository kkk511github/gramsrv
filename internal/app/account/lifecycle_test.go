	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.wantKind == domain.AccountDeleteDelayed {
				if lifecycle.scheduled == nil || !strings.Contains(lifecycle.scheduled.ServiceMessage, "safelink://confirmphone?") || outcome.WaitSeconds != int(accountDeletionDelay.Seconds()) {
