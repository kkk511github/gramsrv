...
func TestSeedDirectoryAppliesSafeLinkBranding(t *testing.T) {
	root := t.TempDir()
...
`), 0o600); err != nil {
	...
	if seeded, err := service.SeedDirectory(context.Background(), root); err != nil || seeded != 4 {
		t.Fatalf("seed = %d, %v; want 4, nil", seeded, err)
	}
	pack, err := service.GetLangPack(context.Background(), "tdesktop", "en")
	if err != nil {
		t.Fatalf("get pack: %v", err)
	}
	if langPackTestValue(pack, "TelegramPremium") != "SafeLink Premium" ||
