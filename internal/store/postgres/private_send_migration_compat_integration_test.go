...
func TestPrivateSendFreshMigrationsKeepLegacyWriterDefaults(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			migration: "migrations/0067_private_message_idempotency.up.sql",
			adds: []string{
		...
		{
			migration: "migrations/0073_private_message_send_receipt.up.sql",
			adds: []string{
...
ALTER TABLE public.private_messages
	...
	upSQL, err := deploy.Migrations.ReadFile("migrations/0082_correct_private_send_defaults.up.sql")
	if err != nil {
