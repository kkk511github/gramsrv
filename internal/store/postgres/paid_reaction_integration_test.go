...
func TestPaidReactionAtomicLedgerMigrationCreditsLegacyAggregatesPostgres(t *testing.T) {
	...
	downSQL, err := deploy.Migrations.ReadFile("migrations/0184_paid_reaction_atomic_ledger.down.sql")
	if err != nil {
		t.Fatalf("read 0178 down: %v", err)
	}
	upSQL, err := deploy.Migrations.ReadFile("migrations/0184_paid_reaction_atomic_ledger.up.sql")
	if err != nil {
