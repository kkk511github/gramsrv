...
func TestAccountFreezeMigrationAndStoreRoundTrip(t *testing.T) {
	...
	downSQL, err := deploy.Migrations.ReadFile("migrations/0094_account_freeze_state.down.sql")
	if err != nil {
		t.Fatal(err)
	}
	upSQL, err := deploy.Migrations.ReadFile("migrations/0094_account_freeze_state.up.sql")
	if err != nil {
		t.Fatal(err)
	}
...
VALUES ($1, true, 'legacy freeze', 'ops', 'legacy-freeze', $2)`, frozenUserID, legacyUpdatedAt); err != nil {
	...
	if !migrated.Frozen || !migrated.Since.Equal(legacyUpdatedAt) ||
		!migrated.Until.Equal(legacyUpdatedAt.Add(7*24*time.Hour)) || migrated.AppealURL != "https://safelink.chat/SpamBot" || migrated.Version != 1 {
