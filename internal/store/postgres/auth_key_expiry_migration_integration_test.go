...
const (
	authKeyExpiryMigrationUp   = "migrations/0092_auth_key_protocol_expiry.up.sql"
	authKeyExpiryMigrationDown = "migrations/0092_auth_key_protocol_expiry.down.sql"
)
...
`, []int64{tempKeyID, permKeyID, authorizedPermKeyID, unknownKeyID}).Scan(
		...
		t.Fatalf("inspect 0092 down result: %v", err)
	}
	if expiresColumnExists || expiryIndexExists || permFKExists || fixtureKeyCount != 4 {
		t.Fatalf("0092 down result column=%v index=%v fk=%v keys=%d, want false/false/false/4", expiresColumnExists, expiryIndexExists, permFKExists, fixtureKeyCount)
	}
}
  ...
  id, target_user_id, target_auth_key_id, chat_id, event_type, date
	...
	for i, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatal("0092 migration accepted invalid identity state")
			}
			var pgErr *pgconn.PgError
			if !errors.As(err, &pgErr) || pgErr.Code != "P0001" || !strings.Contains(pgErr.Message, test.wantMessage) {
				t.Fatalf("0092 migration error = %v, want SQLSTATE P0001 containing %q", err, test.wantMessage)
			}
		})
	}
}
