...
func TestChannelSendFingerprintMigrationRoundTripPostgres(t *testing.T) {
	pool := testPool(t)
	ctx := context.Background()
	downSQL, err := deploy.Migrations.ReadFile("migrations/0083_channel_send_fingerprint.down.sql")
	if err != nil {
		t.Fatalf("read 0078 down: %v", err)
	}
	upSQL, err := deploy.Migrations.ReadFile("migrations/0083_channel_send_fingerprint.up.sql")
	if err != nil {
