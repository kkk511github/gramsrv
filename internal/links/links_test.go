...
func TestNormalizeBaseURL(t *testing.T) {
	tests := []struct {
		...
		{name: "default", raw: "", want: "https://safelink.chat"},
...
func TestAppLinkBuilderPreservesLegacyAndSupportsHostBase(t *testing.T) {
	...
	if got, want := legacy.BuildRoot(), "telesrv://"; got != want {
	...
	if got, want := hosted.BuildRoot(), "owpg://links.example.test"; got != want {
		t.Fatalf("hosted root = %q, want %q", got, want)
	}
