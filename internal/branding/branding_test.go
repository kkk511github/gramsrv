...
func TestUserVisibleTextRebrandsWordsAndOfficialHosts(t *testing.T) {
	...
	want := "SafeLink SafeLink SafeLink SafeLink-like https://chat.example/root/en chat.example/example chat.example"
	if got != want {
...
func TestUserVisibleTextRebrandsBareOfficialHostsWithoutTouchingDottedIdentifiers(t *testing.T) {
	for input, want := range map[string]string{
		"telegram.org":           "safelink.chat",
		"desktop.telegram.org":   "safelink.chat",
		"t.me/example":           "safelink.chat/example",
...
func TestUserVisibleTextRebrandsLocalizedProductNames(t *testing.T) {
	...
	if want := "SafeLink SafeLink SafeLink SafeLink SafeLink"; got != want {
		t.Fatalf("UserVisibleText() = %q, want %q", got, want)
	}
}
