...
func newStickerLinkHandler(t *testing.T, files publicweb.StickerSetResolver) http.Handler {
	t.Helper()
	h, err := publicweb.NewHandler(publicweb.Config{StickerSets: files, PublicBaseURL: "https://safelink.chat"})
	if err != nil {
...
func TestStickersBotCreatePackLinkInstallIsolationSmoke(t *testing.T) {
	...
	waitForStickersReplyAfter(t, messageStore, alice.ID, lastBotReplyID, "https://safelink.chat/addstickers/alice_bot_pack")
