...
func TestAccountCollectibleEmojiStatusListSetAndRejectNonOwner(t *testing.T) {
	...
	if !ok || collectible.CollectibleID != gift.ID || collectible.DocumentID != clientDocumentIDFromServerID(gift.Model.Document.ID) ||
		collectible.PatternDocumentID != clientDocumentIDFromServerID(gift.Pattern.Document.ID) || collectible.PatternColor != gift.Backdrop.PatternColor {
...
func TestCollectibleEmojiStatusDurableUpdateProjection(t *testing.T) {
	...
	if status, ok := update.EmojiStatus.(*tg.EmojiStatusCollectible); !ok || status.PatternDocumentID != clientDocumentIDFromServerID(collectible.PatternDocumentID) {
