...
func TestMessagesEditMessageReturnsUpdateAndRecordsOwnerContext(t *testing.T) {
	...
	req.SetMessage("edited safelink://resolve?domain=Alice")
	...
	if !ok || msg.ID != 3 || msg.Message != "edited safelink://resolve?domain=Alice" {
	...
	if len(messages.editReq.Entities) != 2 || messages.editReq.Entities[0].Type != domain.MessageEntityBold ||
		...
		messages.editReq.Entities[1].Length != utf16CodeUnitLen("safelink://resolve?domain=Alice") {
