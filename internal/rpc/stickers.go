...
func (r *Router) onMessagesGetStickerSet(ctx context.Context, req *tg.MessagesGetStickerSetRequest) (tg.MessagesStickerSetClass, error) {
	...
	if !found {
		if fallbackSet, fallbackDocs, fallbackFound, fallbackErr := r.resolvePlaceholderStickerSet(ctx, ref); fallbackErr != nil {
			...
			responseHash := stickerSetClientHash(fallbackSet)
			if r.log != nil {
				r.log.Debug("getStickerSet placeholder fallback",
					...
					zap.Int("response_hash", responseHash),
				)
			}
			if stickerSetHashMatches(req.Hash, fallbackSet) {
	...
	if stickerSetHashMatches(req.Hash, set) {
		return &tg.MessagesStickerSetNotModified{}, nil
	}
...
func stickerSetHashMatches(requestHash int, set domain.StickerSet) bool {
	if requestHash == 0 {
		return false
	}
	return requestHash == set.Hash || requestHash == stickerSetClientHash(set)
}
