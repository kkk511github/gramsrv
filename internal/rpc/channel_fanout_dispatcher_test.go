...
func TestChannelFanoutDispatcherQueueFullNudgesOnlineMembers(t *testing.T) {
	...
	d.shards[0].jobs <- fanoutTestJob([]int64{9999}, 0, 0, nil)
	...
	queued, blocked := d.drainOneOverflow(d.shards[0])
	if !queued || blocked {
		t.Fatalf("drain overflow queued=%v blocked=%v, want true/false", queued, blocked)
	}
	channelID := <-d.nudgeJobs
	nudge, ok := d.takeNudge(channelID)
	if !ok {
		t.Fatal("queued overflow nudge is missing")
	}
	r.runChannelFanoutOverflowNudge(context.Background(), nudge.channelID, nudge.pts)
