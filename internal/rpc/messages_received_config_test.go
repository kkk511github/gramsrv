...
func TestHelpGetConfigReturnsDefaultAndAccountReaction(t *testing.T) {
	...
	wantDocumentID := clientDocumentIDFromServerID(7001)
	if custom, ok := authorized.ReactionsDefault.(*tg.ReactionCustomEmoji); !ok || custom.DocumentID != wantDocumentID {
		t.Fatalf("authorized reactions_default = %#v, want custom emoji %d", authorized.ReactionsDefault, wantDocumentID)
	}
}
