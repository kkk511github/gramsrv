...
func TestModerationFlagsProjectToChannelWithSafeLinkWarning(t *testing.T) {
	...
	wantAbout := defaultScamWarningChannel + "\n\n" + channel.About
	if full.About != wantAbout {
		t.Fatalf("about=%q, want warning projection %q", full.About, wantAbout)
	}
	if channel.About != "Owner description" {
		t.Fatalf("stored about mutated to %q", channel.About)
	}
}
