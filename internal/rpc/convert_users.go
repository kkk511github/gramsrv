...
func tgUserEmojiStatusValue(value domain.UserEmojiStatus) tg.EmojiStatusClass {
	...
	if collectible := value.Collectible; !collectible.Empty() {
		status := &tg.EmojiStatusCollectible{
			...
			DocumentID:        clientDocumentIDFromServerID(collectible.DocumentID),
			Title:             collectible.Title,
			Slug:              collectible.Slug,
			PatternDocumentID: clientDocumentIDFromServerID(collectible.PatternDocumentID),
	...
	status := &tg.EmojiStatus{DocumentID: clientDocumentIDFromServerID(value.DocumentID)}
	if value.Until > 0 {
		status.SetUntil(value.Until)
	}
	return status
