# SafeLink OTP delivery providers

The SafeLink server owns OTP generation, storage, attempt limits, expiry, verification,
...
The SafeLink server sends one `POST` request and does not follow redirects:
