...
## 2026-08-12 密聊完整链路、媒体搜索、频道统计与网页预览收敛

上游新增原始迁移 `0175_channel_stats_read_model`，与 SafeLink 已发布的
`0175_premium_native_checkout` 重号，合并时必须映射为：

- `0181_channel_stats_read_model`

生产库应从 `180 | false` 升级为 `181 | false`。该迁移为频道浏览者、成员
周期和公开转发查询增加索引，供官方客户端的频道统计页使用。不得把文件改回
上游原编号，否则会与现有 Premium 结算迁移冲突。

- 密聊请求 ID、加密文件 access hash、密聊消息队列和 DH 配置版本现在按
  官方客户端约定一致处理；下载密聊文件必须同时匹配 ID 和 access hash。
- 照片、视频、文档、语音和链接等媒体搜索使用完整类型映射和有界查询，
  减少客户端搜索结果缺失或分页不稳定。
- 频道创建返回、初始 PTS 与管理员统计权限已补齐，TDLib 和 TDesktop 可正确
  识别新频道并显示统计入口。
- 过期的网页预览 pending 状态会最终收敛，历史消息、频道消息和故事投影
  不再长期停留在“正在加载预览”。
- SafeLink 的消息文档 ID 兼容、贴纸缩略图、审核警告、品牌链接、推送和设备
  位置逻辑必须在合并后保持，不能被上游默认投影覆盖。

部署前备份 PostgreSQL，替换二进制并重启 `slerv`，等待启动日志出现
`slerv 服务就绪`或 `2398` 真正开始监听。部署后至少确认：迁移为
`181 | false`、RSA 指纹未变、现有客户端可重连、新建频道正常、频道管理员
可打开统计、密聊文件可下载，以及官网、邀请链接和 Web 均可访问。
