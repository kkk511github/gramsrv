...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'invalid non-positive temporary auth key expiry; repair before migration 0092';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'temporary auth key binding references missing permanent key; repair before migration 0092';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM public.temp_auth_key_bindings
        ...
        RAISE EXCEPTION 'temporary auth key self-binding; repair before migration 0092';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'auth key appears in both temporary and permanent roles; repair before migration 0092';
    END IF;
END
$$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'invalid authorization on temporary auth key; repair before migration 0092';
    END IF;
END
$$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'update state references temporary auth key; repair before migration 0092';
    END IF;
    IF EXISTS (
        ...
        RAISE EXCEPTION 'bootstrap update job references temporary auth key; repair before migration 0092';
    END IF;
    IF EXISTS (
        ...
        RAISE EXCEPTION 'secret qts watermark references temporary auth key; repair before migration 0092';
    END IF;
    IF EXISTS (
        ...
        RAISE EXCEPTION 'encrypted message queue references temporary auth key; repair before migration 0092';
    END IF;
    IF EXISTS (
        ...
        RAISE EXCEPTION 'encrypted state delivery references temporary auth key; repair before migration 0092';
    END IF;
    IF EXISTS (
        ...
        RAISE EXCEPTION 'encrypted state event targets temporary auth key; repair before migration 0092';
    END IF;
    IF EXISTS (
        ...
        RAISE EXCEPTION 'secret chat references temporary auth key; repair before migration 0092';
    END IF;
END
$$;
