#!/usr/bin/env python3
"""Bedolaga-style aiogram formatted-text demo for the SafeLink Bot API.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bedolaga-style aiogram HTML/Markdown demo against SafeLink"
    )
    ...
    parser.add_argument(
        ...
        help="serve and send the Bedolaga SafeLink Login/OIDC demo",
    )
    parser.add_argument(
        "--login-issuer",
...
async def send_login_demo(bot: Bot, chat_id: int, marker: str, config: LoginDemoConfig) -> int:
    ...
    LOG.info("sent SafeLink Login demo chat_id=%s message_id=%s", chat_id, message.message_id)
...
def build_dispatcher(marker: str, login_config: LoginDemoConfig | None = None) -> Dispatcher:
    router = Router(name="safelink-bedolaga-format")
    ...
    async def login_demo(message: Message) -> None:
        if login_config is None:
            await message.answer(
                "<b>SafeLink Login demo is disabled.</b> Start this program with "
