# Bedolaga formatted-text + SafeLink Login demo
...
SafeLink 发送 `parse_mode=HTML`。`/formatdemo` 依次发送默认 HTML、legacy Markdown、
MarkdownV2，用于验证完整的 `aiogram → SafeLink Bot API → MTProto message/update →
TDesktop` 链路。
