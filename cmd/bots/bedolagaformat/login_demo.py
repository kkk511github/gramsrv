"""Local Bedolaga SafeLink Login/OIDC relying-party demo.
...
* the self-hosted SafeLink Login JavaScript SDK ``post_message`` response; and
...
class LoginDemoServer:
    ...
    async def start(self) -> None:
        ...
        await site.start()
        LOG.info(
            "SafeLink Login demo listening at %s (issuer=%s client_id=%s)",
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Bedolaga SafeLink Login Demo</title>
...
<body><main class="card"><h1>Bedolaga SafeLink Login Demo</h1>{legacy_result}
