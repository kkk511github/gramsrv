...
test("Chinese, Russian and English dictionaries have the same complete key set", () => {
...
test("language normalization, commands and errors follow the selected language", () => {
  ...
  assert.equal(commandList("zh")[0].description, "打开主菜单");
...
test("catalog titles and invoice descriptions are localized", () => {
  ...
  assert.equal(localizeProduct(product, "en").title, "SafeLink Premium — 1 month");
  assert.equal(localizeProduct(product, "zh").title, "SafeLink Premium - 1 个月");
