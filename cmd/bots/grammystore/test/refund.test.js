...
test("Stars reversal debits the snapshotted grant with a deterministic key", async () => {
  ...
  assert.deepEqual(calls, [[1001, 20, "SafeLink bot refund", "refund:charge-1:stars"]]);
});
...
test("Premium reversal only revokes the entitlement created by the purchase", async () => {
  ...
  await reverseSaleFulfillment(sale, db, gramsrv);
  assert.deepEqual(calls, [[1001, 77, "SafeLink bot refund", "refund:charge-premium:premium"]]);
});
...
test("Bot API retry does not debit the internal product twice", async (t) => {
  const dir = mkdtempSync(path.join(tmpdir(), "safelink-refund-"));
  const db = new BotDatabase(path.join(dir, "bot.sqlite3"));
  t.after(() => { db.close(); rmSync(dir, { recursive: true, force: true }); });
  db.addSale({
    product: "stars_1", title: "20 SafeLink Stars", starsPrice: 1, recipientID: 1001,
  ...
  await assert.rejects(() => executeCompensatedRefund({
    ...
    refundStarPayment: async () => { throw new Error("temporary Bot API failure"); },
  }), /temporary/);
