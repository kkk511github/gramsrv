...
export async function reverseSaleFulfillment(sale, db, gramsrv) {
  ...
  if (item.kind === "stars") await gramsrv.debitStars(item.recipientID, item.amount, "SafeLink bot refund", key);
    ...
    await gramsrv.revokePremium(item.recipientID, item.entitlementID, "SafeLink bot refund", key);
...
export function mainKeyboard(language, admin = false, paymentsEnabled = false) {
  const kb = new InlineKeyboard();
  if (paymentsEnabled) kb.text(translate(language, "buttonShop"), "menu:shop").row();
  kb.text(translate(language, "buttonBonuses"), "menu:bonuses").text(translate(language, "buttonReferrals"), "menu:referrals").row()
...
export function shopKeyboard(language) {
  return new InlineKeyboard()
    ...
    .text(translate(language, "buttonUsername"), "shop:username").row()
...
export function settingsKeyboard(language, user) {
  ...
  const chinese = `${current === "zh" ? "✅ " : ""}${translate(language, "languageChinese")}`;
  ...
  return new InlineKeyboard()
    ...
    .text(chinese, "settings:lang:zh").row()
...
export function adminKeyboard(language, paymentsEnabled = false) {
  const kb = new InlineKeyboard()
    ...
    .text(translate(language, "adminBonusButton"), "admin:bonus").row();
  if (paymentsEnabled) kb.text(translate(language, "adminInvoiceButton"), "admin:invoice").text(translate(language, "adminRefundButton"), "admin:refund").row();
  return kb
...
export function createBot({ config, db, gramsrv }) {
  ...
  bot.command("start", async (ctx) => {
    ...
    await ctx.reply(lines.join("\n\n"), { parse_mode: "HTML", reply_markup: mainKeyboard(language, isOwner(config, ctx.from.id), config.paymentsEnabled) });
  });

  bot.command("menu", async (ctx) => {
    const language = languageOf(ctx.from.id);
    await editOrReply(ctx, tr(ctx.from.id, "menuTitle"), mainKeyboard(language, isOwner(config, ctx.from.id), config.paymentsEnabled));
  });

  bot.command("admin", async (ctx) => {
    if (!isOwner(config, ctx.from.id)) return;
    const language = languageOf(ctx.from.id);
    await editOrReply(ctx, tr(ctx.from.id, "adminTitle"), adminKeyboard(language, config.paymentsEnabled));
  });
  ...
  async function fulfill(product, recipientID, buyer, chatID, chargeID, extra = "") {
    ...
    if (product.kind === KINDS.premium) {
      const result = await gramsrv.grantPremium(recipientID, product.months, "SafeLink bot purchase", key);
      ...
      await gramsrv.grantStars(recipientID, product.starsAmount, "SafeLink bot purchase", key);
  ...
  bot.callbackQuery(/^subscription:check$/, async (ctx) => {
    ...
    const language = languageOf(ctx.from.id);
    if (await subscribed(ctx, config)) await editOrReply(ctx, tr(ctx.from.id, "menuTitle"), mainKeyboard(language, isOwner(config, ctx.from.id), config.paymentsEnabled));
  ...
  bot.callbackQuery(/^menu:(.+)$/, async (ctx) => {
    ...
    const language = languageOf(ctx.from.id);
    if (page === "home") return editOrReply(ctx, tr(ctx.from.id, "menuTitle"), mainKeyboard(language, isOwner(config, ctx.from.id), config.paymentsEnabled));
    if (page === "numbers") {
      if (!config.paymentsEnabled) return editOrReply(ctx, tr(ctx.from.id, "menuTitle"), mainKeyboard(language, isOwner(config, ctx.from.id), false));
    ...
    if (page === "shop") {
      if (!config.paymentsEnabled) return editOrReply(ctx, tr(ctx.from.id, "menuTitle"), mainKeyboard(language, isOwner(config, ctx.from.id), false));
      return editOrReply(ctx, tr(ctx.from.id, "shopTitle"), shopKeyboard(language));
    }
    if (page === "bonuses") {
    ...
    if (page === "referrals") {
      ...
      const link = `${config.publicBaseURL}/${username}?start=ref_${ctx.from.id}`;
  ...
  bot.callbackQuery(/^settings:lang:(zh|ru|en)$/, async (ctx) => {
  ...
  bot.callbackQuery(/^admin:(.+)$/, async (ctx) => {
    ...
    const language = languageOf(ctx.from.id);
    if (action === "menu") return editOrReply(ctx, tr(ctx.from.id, "adminTitle"), adminKeyboard(language, config.paymentsEnabled));
    if (action === "stats") {
    ...
    if (!config.paymentsEnabled && ["invoice", "access", "refund"].includes(action)) {
      return editOrReply(ctx, tr(ctx.from.id, "adminTitle"), adminKeyboard(language, false));
    }
  ...
  bot.on("message:text", async (ctx) => {
    ...
    try {
      if (pending.kind === "account") {
        ...
        return ctx.reply(tr(ctx.from.id, "accountSaved", { id }), { parse_mode: "HTML", reply_markup: mainKeyboard(language, isOwner(config, ctx.from.id), config.paymentsEnabled) });
      }
      if (pending.kind === "stars_amount") {
        if (!config.paymentsEnabled) { db.clearPending(ctx.from.id); return; }
      ...
      if (pending.kind === "admin_stars") {
        ...
        await gramsrv.grantStars(id, amount, "SafeLink bot administrator grant", pending.payload.operationID); db.clearPending(ctx.from.id);
      ...
      if (pending.kind === "admin_premium") {
        ...
        await gramsrv.grantPremium(id, months, "SafeLink bot administrator grant", pending.payload.operationID); db.clearPending(ctx.from.id);
      ...
      if (pending.kind === "admin_access") {
        ...
        if (!phone || !Number.isSafeInteger(telegramID) || telegramID <= 0) throw new Error("invalid phone or bot user ID");
